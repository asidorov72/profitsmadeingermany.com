...
class Edigital_About {
	...
	public function admin_menu() {
		...
		$page = add_theme_page( esc_html__( 'About', 'edigital' ) . ' ' . $theme->display( 'Name' ), esc_html__( 'About', 'edigital' ) . ' ' . $theme->display( 'Name' ), 'activate_plugins', 'edigital-welcome', array( $this, 'welcome_screen' ) );
	...
	public function welcome_notice() {
		...
		<div id="message" class="updated edigital-message">
			...
			<p><?php printf( esc_html__( 'Welcome! Thank you for choosing edigital! To fully take advantage of the best our theme can offer please make sure you visit our %swelcome page%s.', 'edigital' ), '<a href="' . esc_url( admin_url( 'themes.php?page=edigital-welcome' ) ) . '">', '</a>' ); ?></p>
			<p class="submit">
				<a class="button-secondary" href="<?php echo esc_url( admin_url( 'themes.php?page=edigital-welcome' ) ); ?>"><?php esc_html_e( 'Get started with edigital', 'edigital' ); ?></a>
			</p>
		</div>
		<?php
	}
	...
	private function intro() {
		...
		<h2 class="nav-tab-wrapper">
			<a class="nav-tab <?php if ( empty( $_GET['tab'] ) && $_GET['page'] == 'edigital-welcome' ) echo 'nav-tab-active'; ?>" href="<?php echo esc_url( admin_url( add_query_arg( array( 'page' => 'edigital-welcome' ), 'themes.php' ) ) ); ?>">
				<?php echo $theme->display( 'Name' ); ?>
			</a>
			
			<a class="nav-tab <?php if ( isset( $_GET['tab'] ) && $_GET['tab'] == 'free_vs_pro' ) echo 'nav-tab-active'; ?>" href="<?php echo esc_url( admin_url( add_query_arg( array( 'page' => 'edigital-welcome', 'tab' => 'free_vs_pro' ), 'themes.php' ) ) ); ?>">
				<?php esc_html_e( 'Free Vs Pro', 'edigital' ); ?>
			</a>

			<a class="nav-tab <?php if ( isset( $_GET['tab'] ) && $_GET['tab'] == 'more_themes' ) echo 'nav-tab-active'; ?>" href="<?php echo esc_url( admin_url( add_query_arg( array( 'page' => 'edigital-welcome', 'tab' => 'more_themes' ), 'themes.php' ) ) ); ?>">
				<?php esc_html_e( 'More Themes', 'edigital' ); ?>
			</a>

			<a class="nav-tab <?php if ( isset( $_GET['tab'] ) && $_GET['tab'] == 'changelog' ) echo 'nav-tab-active'; ?>" href="<?php echo esc_url( admin_url( add_query_arg( array( 'page' => 'edigital-welcome', 'tab' => 'changelog' ), 'themes.php' ) ) ); ?>">
			...
			<div class="changelog point-releases">
				<div class="under-the-hood two-col">
					<div class="col">
						...
						<p><a href="<?php echo admin_url( 'customize.php' ); ?>" class="button button-secondary"><?php esc_html_e( 'Customize', 'edigital' ); ?></a></p>
					</div>
					...
					<div class="col">
						...
						<p>
							<a href="<?php echo esc_url( 'http://translate.wordpress.org/projects/wp-themes/edigital' ); ?>" class="button button-secondary" target="_blank">
								<?php
								esc_html_e( 'Translate', 'edigital' );
								echo ' ' . $theme->display( 'Name' );
								?>
						...
						foreach ( $themes->themes as $theme ) {
							if( $active_theme == $theme->name ) { ?>
								...
								<div id="<?php echo $theme->slug; ?>" class="theme active">
									...
									<h3 class="theme-name" id="edigital-name"><strong><?php _e( 'Active', 'edigital' ); ?></strong>: <?php echo $theme->name; ?></h3>
									<div class="theme-actions">
										<a class="button button-primary customize load-customize hide-if-no-customize" href="<?php echo get_site_url(). '/wp-admin/customize.php' ?>"><?php _e( 'Customize', 'edigital' ); ?></a>
									</div>
								</div><!-- .theme active -->
							<?php
							$counter++;
									...
									<div class="theme-actions">
										<?php if( wp_get_theme( $theme->slug )->exists() ) { ?>											
											...
											<a  class="button button-secondary activate"
												href="<?php echo wp_nonce_url( admin_url( 'themes.php?action=activate&amp;stylesheet=' . urlencode( $theme->slug ) ), 'switch-theme_' . $theme->slug );?>" ><?php _e( 'Activate', 'edigital' ) ?></a>
										<?php } else {
										...
										?>
											<!-- Install Button -->
											<a data-toggle="tooltip" data-placement="bottom" title="<?php echo 'Downloaded ' . number_format( $theme_details->downloaded ) . ' times'; ?>" class="button button-secondary activate" href="<?php echo esc_url( wp_nonce_url( $install_url, 'install-theme_' . $theme->slug ) ); ?>" ><?php _e( 'Install Now', 'edigital' ); ?></a>
										<?php } ?>

										<a class="button button-primary load-customize hide-if-no-customize" target="_blank" href="<?php echo $theme->preview_url; ?>"><?php _e( 'Live Preview', 'edigital' ); ?></a>
									</div>
								</div><!-- .theme -->
								<?php
							}
