...
function hss_edd_add_defaults() {
    ...
    if(($tmp['chk_default_options_db']=='1')||(!is_array($tmp))) {
                ...
                $arr = array(   "api_key" => "","jwplayer_stretching" => "uniform","logging" => "NORMAL", "database_id" => "0", "disable_desc_updates" => 0, "jwplayer_version" => "7", "jwplayer_logo_file" => "", "jwplayer_logo_link" => "", "jwplayer_logo_hide" => "false" );
...
add_action('wp_head', 'hss_edd_javascript', 9999);

function hss_edd_javascript() {
?>
<script type="text/javascript" >
	jQuery(document).ready(function($) {
                ...
                <form method="post" action="options.php">
                        ...
                        <table class="form-table">
                                ...
                                <tr>
                                        ...
                                        <td>
                                                <select name="hss_options[jwplayer_version]">
                                                <?php
                                ...
                                <tr>
                                        <th scope="row">JW Player License Key<BR><i>(available from <a href='http://www.jwplayer.com' target='_blank'>www.jwplayer.com</a>)</i></th>
                                        <td>
                                ...
                                <tr>
                                        ...
                                        <td>
                                                <input type="text" size="50" name="hss_options[jwplayer_logo_file]" value="<?php echo $options['jwplayer_logo_file']; ?>" />
                                        </td>
                                </tr>
                                <tr>
                                        <th scope="row">JW Player Watermark Link<BR><i>(JW Player 7 only)</i></th>
                                        <td>
                                                <input type="text" size="50" name="hss_options[jwplayer_logo_link]" value="<?php echo $options['jwplayer_logo_link']; ?>" />
                                        </td>
                                </tr>
                                <tr>
                                        <th scope="row">JW Player Watermark Hide<BR><i>(JW Player 7 only)</i></th>
                                        <td>
                                                <select name="hss_options[jwplayer_logo_hide]">
                                                <?php
	...
	_log("hss_edd_complete_purchase_add_video - ".$payment_id." - ".$new_status." - ".$old_status);
		...
		_log($user_info);
			...
			if((get_post_meta($download['id'], 'is_streaming_video', true)) or (get_post_meta($download['id'], 'is_streaming_video_bundle', true))) {
				...
				}elseif(array_key_exists('price_id',$download['options'])){
					if (empty($download['options']['price_id'])){ 
                                               $ppv_option = get_post_meta($download['id'], '_edd_ppv_id', true);
                                        }else{
						if ($download['options']['price_id'] != 0)
