...
class PostType extends Model {
	...
	static function getInstance($post) {
		if (is_scalar($post)) {
			if (!empty(static::$instances[$post]) AND (!static::POST_TYPE OR static::$instances[$post]->getPostType() == static::POST_TYPE)) {
				return static::$instances[$post];
			}
			else if (is_numeric($post)) {
				$post = get_post($post);
			} else {
				$post = get_post(array('post_name' => $post, 'post_status' => 'any', 'post_type' => (!static::POST_TYPE ? null : static::POST_TYPE)));
			}
		}
		if (!empty($post) AND is_object($post) AND (!static::POST_TYPE OR $post->post_type == static::POST_TYPE)) {
...
// 			var_dump($result);
	...
	function getEditUrl() {
		return add_query_arg(array('post' => $this->getId(), 'action' => 'edit'), admin_url('post.php'));
	}
	
	
	function getFeaturedImage($size = 'post-thumbnail', $attr = array()) {
		
		if ($post_thumbnail_id = get_post_thumbnail_id( $this->post )) {
			if ($image = wp_get_attachment_image_src($post_thumbnail_id, $size, $icon = false)) {
				return $image[0];
			}
		}
