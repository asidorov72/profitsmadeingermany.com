		...
		<table>
			<thead><tr>
				...
				<?php if (is_user_logged_in() AND $atts['status'] != 'active'): ?>
					<th><?php echo Labels::getLocalized('lesson_purchase'); ?></th>
				<?php endif; ?>
			</tr></thead>
			<tbody>
				<?php foreach ($rows as $row): ?>
					...
					<tr>
						<td data-col="channel-name">
							...
							<div data-field="channel-total-videos"><strong><?php echo Labels::getLocalized('lesson_videos_num'); ?>:</strong>
								<span><?php echo $channel->getTotalVideos(); ?></span></div>
							<div data-field="channel-duration-min"><strong><?php echo Labels::getLocalized('lesson_duration'); ?>:</strong>
								<span><?php echo TimeHelper::niceTimeFormat($channel->getDurationSec()); ?></span></div>
						</td>
