...
class PostSubscription extends Model {
	...
	function getPaymentModel() {
		$micropayments = new Micropayments($this->post);
		if ($micropayments->isAvailable() AND $micropayments->isPayed()) return $micropayments;
		$instantPayments = new PostEDDPayment($this->post);
	...
	static function isAvailable() {
		return (Micropayments::isAvailable() OR PostEDDPayment::isAvailable());
	}
		...
		if (Settings::getOption(Settings::OPTION_NEW_SUB_ADMIN_NOTIF_ENABLE) AND $user = get_userdata($userId)) {
			...
			Email::send(
				...
				array(
					...
					'[channelname]' => $this->post->getTitle(), // for the compatibility
					'[lessonname]' => $this->post->getTitle(),
					...
					'[duration]' => PostSubscription::seconds2period($periodSeconds),
					'[points]' => $amountPaid,
					'[amount]' => $amountPaid,
	...
	function isSubscriptionActive($userId = null) {
		...
		switch (Settings::getOption(Settings::OPTION_EDD_PAYMENT_MODEL)) {
			...
			default:
				$where .= $wpdb->prepare("s.post_id = %d", $this->post->getId());
		}
